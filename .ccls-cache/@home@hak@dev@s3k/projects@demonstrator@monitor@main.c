...
{
	s3k_cap_t pmp;
	pmp = s3k_mk_pmp(s3k_napot_encode((uint64_t)APP0_MEM, APP0_MEM_LEN), S3K_MEM_RW);
	s3k_cap_derive(0x5, 0x10, pmp);
	s3k_pmp_load(0x10, 4);
	s3k_sync_mem();

	pmp = s3k_mk_pmp(s3k_napot_encode((uint64_t)APP0_MEM, APP0_MEM_LEN), S3K_MEM_RWX);
	...
	pmp = s3k_mk_pmp(s3k_napot_encode((uint64_t)APP1_MEM, APP1_MEM_LEN), S3K_MEM_RW);
	s3k_cap_derive(0x6, 0x11, pmp);
	s3k_pmp_load(0x11, 5);
	s3k_sync_mem();

	pmp = s3k_mk_pmp(s3k_napot_encode((uint64_t)APP1_MEM, APP1_MEM_LEN), S3K_MEM_RWX);
...
void reset_app(s3k_msg_t *msg, app_data_t *app)
{
	s3k_mon_reg_write(MONITOR_CIDX, app->pid, S3K_REG_PC, (uint64_t)app->memory);
	msg->data[0] = 2;
	memcpy(uart_buffer, "ok", 2);
}
