...
s3k_err_t s3k_mon_cap_move(s3k_cidx_t mon_idx, s3k_pid_t src_pid, s3k_cidx_t src_idx, s3k_pid_t dst_pid,
			   s3k_cidx_t dst_idx)
{
	s3k_err_t err;
	do {
...
s3k_err_t s3k_mon_pmp_load(s3k_cidx_t mon_idx, s3k_pid_t pid, s3k_cidx_t pmp_idx, s3k_pmp_slot_t pmp_slot)
{
	s3k_err_t err;
	do {
...
s3k_err_t s3k_try_mon_reg_read(s3k_cidx_t mon, s3k_pid_t pid, s3k_reg_t reg, uint64_t *val)
{
	...
	s3k_ret_t ret = DO_ECALL(S3K_SYS_MON_REG_READ, args, sizeof(args.mon_reg_read));
	*val = ret.val;
	return ret.err;
}
...
s3k_err_t s3k_try_mon_reg_write(s3k_cidx_t mon, s3k_pid_t pid, s3k_reg_t reg, uint64_t val)
{
	...
	s3k_ret_t ret = DO_ECALL(S3K_SYS_MON_REG_WRITE, args, sizeof(args.mon_reg_write));
	return ret.err;
}
...
s3k_err_t s3k_try_mon_cap_read(s3k_cidx_t mon_idx, s3k_pid_t pid, s3k_cidx_t idx, s3k_cap_t *cap)
{
	...
	s3k_ret_t ret = DO_ECALL(S3K_SYS_MON_CAP_READ, args, sizeof(args.mon_cap_read));
	if (!ret.err)
		cap->raw = ret.val;
	return ret.err;
}

s3k_err_t s3k_try_mon_cap_move(s3k_cidx_t mon_idx, s3k_pid_t src_pid, s3k_cidx_t src_idx, s3k_pid_t dst_pid,
			       s3k_cidx_t dst_idx)
{
	sys_args_t args = {
	    .mon_cap_move = {mon_idx, src_pid, src_idx, dst_pid, dst_idx}
	 };
	return DO_ECALL(S3K_SYS_MON_CAP_MOVE, args, sizeof(args.mon_cap_move)).err;
}

s3k_err_t s3k_try_mon_pmp_load(s3k_cidx_t mon_idx, s3k_pid_t pid, s3k_cidx_t idx, s3k_pmp_slot_t slot)
{
	sys_args_t args = {
...
s3k_err_t s3k_try_mon_pmp_unload(s3k_cidx_t mon_idx, s3k_pid_t pid, s3k_cidx_t idx)
{
	sys_args_t args = {
	    .mon_pmp_unload = {mon_idx, pid, idx}
	 };
	return DO_ECALL(S3K_SYS_MON_PMP_UNLOAD, args, sizeof(args.mon_pmp_unload)).err;
}
