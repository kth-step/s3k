...
{
	s3k_cap_t cap = s3k_mk_memory((uint64_t)base, (uint64_t)base + len, rwx);
	return s3k_cap_derive(MEMORY_RAM_CIDX, dst, cap);
}

s3k_err_t derive_pmp(s3k_cidx_t src, s3k_cidx_t dst, char *base, size_t len, s3k_mem_perm_t rwx)
{
...
void setup_bin(s3k_pid_t pid, char *mem, size_t mem_len, char *bin, size_t bin_len)
{
...
{
	derive_pmp(MEMORY_UART_CIDX, 0x10, (void *)UART0_BASE_ADDR, 0x1000, S3K_MEM_RW);
...
{
	s3k_cap_derive(TIME_HART0_CIDX, 0x10, s3k_mk_time(S3K_MIN_HART, 0, UART_TIME));
	s3k_cap_derive(TIME_HART0_CIDX, 0x11, s3k_mk_time(S3K_MIN_HART, UART_TIME, S3K_SLOT_CNT));
	s3k_cap_derive(0x10, 0x12, s3k_mk_time(S3K_MIN_HART, 0, UART_TIME - 2));
	s3k_cap_derive(0x11, 0x13, s3k_mk_time(S3K_MIN_HART, UART_TIME, S3K_SLOT_CNT - 2));
...
void main(void)
{
	...
	derive_pmp(MEMORY_UART_CIDX, 0x5, (void *)UART0_BASE_ADDR, 0x1000, S3K_MEM_RW);
	s3k_pmp_load(5, 1);
	s3k_sync_mem();

	alt_puts("{setting up memory ...}");
	/* Copy binary of monitor process, setup PMP and program counter. */
	setup_bin(MONITOR_PID, MONITOR_MEM, MONITOR_MEM_LEN, monitor_bin, monitor_bin_len);
	/* Copy binary of crypto process, setup PMP and program counter. */
	setup_bin(CRYPTO_PID, CRYPTO_MEM, CRYPTO_MEM_LEN, crypto_bin, crypto_bin_len);
