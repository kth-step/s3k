...
{
	return (c.type == S3K_CAPTY_MONITOR) && is_range_subset(p.mon.bgn, p.mon.end, c.mon.bgn, c.mon.end);
}
...
static bool s3k_cap_chan_revokable(s3k_cap_t p, s3k_cap_t c)
{
	...
	return (c.type == S3K_CAPTY_CHANNEL) && is_range_subset(p.chan.bgn, p.chan.end, c.chan.bgn, c.chan.end);
}
...
bool s3k_cap_is_valid(s3k_cap_t c)
{
	...
	case S3K_CAPTY_SOCKET:
		return is_bit_subset(c.sock.perm, S3K_IPC_SDATA | S3K_IPC_CDATA | S3K_IPC_SCAP | S3K_IPC_CCAP)
...
static bool s3k_cap_mem_derivable(s3k_cap_t p, s3k_cap_t c)
{
	if (c.type == S3K_CAPTY_PMP) {
		...
		p_end = s3k_tag_block_to_addr(p.mem.tag, p.mem.end);
		s3k_napot_decode(c.pmp.addr, &c_bgn, &c_end);
		return is_range_subset(p_mrk, p_end, c_bgn, c_end) && is_bit_subset(c.pmp.rwx, p.mem.rwx);
	}
	return (c.type == S3K_CAPTY_MEMORY) && (p.mem.tag == c.mem.tag)
...
{
	return (c.type == S3K_CAPTY_MONITOR) && is_range_subset(p.mon.mrk, p.mon.end, c.mon.bgn, c.mon.end);
}

static bool s3k_cap_chan_derivable(s3k_cap_t p, s3k_cap_t c)
{
	if (c.type == S3K_CAPTY_SOCKET) {
		return (c.sock.tag == 0) && is_range_subset(p.chan.mrk, p.chan.end, c.sock.chan, c.sock.chan + 1);
	}
	return (c.type == S3K_CAPTY_CHANNEL) && is_range_subset(p.chan.mrk, p.chan.end, c.chan.bgn, c.chan.end);
}

static bool s3k_cap_sock_derivable(s3k_cap_t p, s3k_cap_t c)
{
	return (c.type == S3K_CAPTY_SOCKET) && (p.sock.chan == c.sock.chan) && (p.sock.tag == 0) && (c.sock.tag != 0)
