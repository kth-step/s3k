...
void cap_snprint(char *restrict buf, size_t size, cap_t cap)
{
	...
	case CAPTY_NONE:
		...
		break;
	case CAPTY_TIME:
		alt_snprintf(buf, size, "TIME{hart=%x,bgn=%x,end=%x,mrk=%x}",
	...
	case CAPTY_MEMORY: {
		...
		alt_snprintf(buf, size,
			     "MEMORY{bgn=0x%X,end=0x%X,mrk=0x%X,rwx=%x,lck=%x}",
	...
	case CAPTY_PMP: {
		...
		alt_snprintf(buf, size,
			     "PMP{bgn=0x%X,end=0x%X,rwx=%x,used=%x,slot=%x}",
			     base, base + _size, cap.pmp.rwx, cap.pmp.used,
			     cap.pmp.slot);
	} break;
	case CAPTY_MONITOR:
		alt_snprintf(buf, size, "MONITOR{bgn=%x,end=%x,mrk=%x}",
			     cap.mon.bgn, cap.mon.end, cap.mon.mrk);
		break;
	case CAPTY_CHANNEL:
		alt_snprintf(buf, size, "CHANNEL{bgn=%x,end=%x,mrk=%x}",
			     cap.chan.bgn, cap.chan.end, cap.chan.mrk);
		break;
	case CAPTY_SOCKET:
		alt_snprintf(buf, size,
			     "SOCKET{chan=%x,tag=%x,perm=%x,mode=%x}",
