...
static bool cap_mem_revokable(cap_t p, cap_t c)
{
	...
	if (c.type == CAPTY_PMP) {
		...
		uint64_t p_bgn = p_tag + ((uint64_t)p.mem.mrk << S3K_MIN_BLOCK_SIZE);
		uint64_t p_end = p_tag + ((uint64_t)p.mem.end << S3K_MIN_BLOCK_SIZE);
...
static bool cap_mem_derivable(cap_t p, cap_t c)
{
	...
	if (c.type == CAPTY_PMP) {
		uint64_t p_tag = (uint64_t)p.mem.tag << S3K_MAX_BLOCK_SIZE;
		uint64_t p_mark = p_tag + ((uint64_t)p.mem.mrk << S3K_MIN_BLOCK_SIZE);
		uint64_t p_end = p_tag + ((uint64_t)p.mem.end << S3K_MIN_BLOCK_SIZE);
		uint64_t c_bgn, c_end;
		pmp_napot_decode(c.pmp.addr, &c_bgn, &c_end);
