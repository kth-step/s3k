...
{
	return parent.free <= child.begin && child.end <= parent.end && child.begin == child.free
	       && parent.offset == child.offset && child.begin <= child.end && (parent.rwx & child.rwx) == child.rwx
	       && parent.lock == 0;
}
...
{
	return parent.type == CAPTY_TIME && child.type == CAPTY_TIME && cap_time_derive_time(parent.time, child.time);
}
...
{
	return parent.type == CAPTY_TIME && child.type == CAPTY_TIME && cap_time_parent_time(parent.time, child.time);
}
