...
void interfer(void)
{
	...
	for (int i = 0; i < rand(); i++) {
		switch (rand() % 8) {
		case 0: s3k_get_pid(); break;
		case 1: s3k_sync(); break;
		case 2: s3k_tsl_revoke(rand() % 32); break;
		case 3: s3k_mon_revoke(rand() % 32); break;
		case 4: s3k_mem_revoke(rand() % 32); break;
		case 5: s3k_ipc_revoke(rand() % 32); break;
		case 6: s3k_tsl_derive(rand() % 32, rand() % 8, rand(), rand() % 8); break;
		case 7: s3k_mem_derive(rand() % 32, rand() % 8, rand(), rand() << 12, rand() << 12); break;
		}
	}
}
